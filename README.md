...
版本从 `1.0.x` 跳跃到 `4.0.0` 的原因：某次发布时误将版本号写为 `3.9.37`，导致 ClawHub 的 `latest` tag 被该错误版本占据（semver 排序 `3 > 1`）。为重置 `latest` tag 至正确版本链，后续版本统一跳至 `4.x.x`。

历史版本：
- `1.0.0` - 初始发布
- `1.0.4` - 最后 1.0 系列版本
- `3.9.37` - ❌ 手滑版本（请勿安装）
- `4.0.0` - 版本号重置版，功能同 `1.0.4` 一致
